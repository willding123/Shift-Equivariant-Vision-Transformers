...
class PolyOrder(torch.autograd.Function):
    ...
    def forward (ctx, x, grid_size, patch_size, norm=2, use_gpu = True):
        ...
        # tmp = torch.permute(tmp, (0,1,2,3,5,4))
        # tmp = torch.permute(tmp , (0,1,4,3,2,5))
        # tmp = tmp.contiguous()
        # tmp = tmp.view(B,C,grid_size[0]*grid_size[1], patch_size[0]*patch_size[1])
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.grid_size = (input_resolution[0] // window_size, input_resolution[1] // window_size)
    ...
    def forward(self, x):
        ...
        x = torch.permute(x, (0,2,3,1)).contiguous() # B,H,W,C
        shortcut = x.view(B,L,C)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if downsample is not None:
            ...
            self.downsample = downsample(input_resolution = input_resolution, patch_size = 2, in_chans = dim, out_chans = dim*2, norm_layer= norm_layer)
        else:
...
class PolySwin(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = PolyPatch(input_resolution = img_size, patch_size = patch_size, in_chans = in_chans, out_chans = embed_dim, norm_layer=norm_layer if self.patch_norm else None)
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               norm_layer=norm_layer,
                               downsample=PolyPatch if (i_layer < self.num_layers - 1) else None,
